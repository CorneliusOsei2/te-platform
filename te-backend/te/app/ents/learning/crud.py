...
def read_sys_design_lesson(
    ...
    return db.query(learning_models.SystemDesign).offset(skip).limit(limit).all()


def read_miscellaneous_lessons(
    db: Session, *, skip: int = 0, limit: int = 100
) -> list[learning_models.Miscellaneous]:
    return db.query(learning_models.Miscellaneous).offset(skip).limit(limit).all()
...
def create_dsa_lesson(
    db: Session, *, author: str, data: learning_schema.DataStructuresAndAlgorithmsCreate
) -> models.User:
    data.author = author
